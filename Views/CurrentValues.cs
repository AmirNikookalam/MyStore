...
namespace Shop.Views
{
    ...
    public class CustomOptionTagHelper : TagHelper
    {
        ...
        public CustomOptionTagHelper(
            ...
            IShopRepository bankRepository)
        {
            _shopRepository = bankRepository;
            Generator = generator;
        }
        ...
        private IEnumerable<SelectListItem> GetItems()
        {
            if (string.Equals(this.EntityName, "BankModel", StringComparison.OrdinalIgnoreCase))
                return _shopRepository.GetProductsAsync().Result.Select(r => new SelectListItem() { Value = r.Id.ToString(), Text = r.Name });
            else
                return null;
        }
    }
